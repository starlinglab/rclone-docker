...
ln -s /data/config $HOME/.config/rclone
...
while [ 1 ]
do
  rclone copy /data/sync/shared-output dropbox:$DROPBOX_PATH_OUTPUT --create-empty-src-dirs
  touch /data/sync/shared-input/.rclone_lock
  rclone copy dropbox:$DROPBOX_PATH_INPUT /data/sync/shared-input --create-empty-src-dirs
